...
import type { IntakeTarget, EligibilityEntry } from "@/lib/apply/intake";
...
export function ReadinessRail({ targets, eligibility }: Props) {
  return (
      ...
      <ul className="mt-4 space-y-2">
        {targets.map((t) => {
          const e = eligibility?.find(
            (x) => x.target.source === t.source && x.target.externalId === t.externalId,
          );
          const researching = t.found === false;
          const ready = e?.ready ?? false;
          const blockers = e?.blockers?.length ?? 0;
