        ...
        <div className="fixed inset-0 z-[10000]" role="dialog" aria-modal="true">
          ...
          <div className="absolute inset-0 grid place-items-center p-4">
            <motion.div
              ...
              className="relative w-full max-w-md rounded-2xl border border-on-surface/8 bg-surface-container-lowest p-6 qc-soft-shadow"
            >
              <button
                type="button"
                onClick={close}
                aria-label="Close"
                className="absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-lg border border-on-surface/10 bg-surface text-on-surface/70 transition-colors hover:bg-on-surface/5 hover:text-on-surface"
              >
                <X className="h-4 w-4" />
              </button>

              <div className="flex items-center gap-2">
                <span className="grid h-9 w-9 place-items-center rounded-lg bg-secondary-fixed text-on-secondary-fixed-variant">
              ...
              <div className="mt-5 rounded-xl border border-on-surface/8 bg-surface-container p-4">
              ...
              {msg && (
                <p
                  className={`mt-4 text-label-sm ${
                    done ? "text-on-surface/80" : "text-error"
                  }`}
                >
                  {msg}
                </p>
              ...
              <div className="mt-5 flex flex-col gap-2">
                ...
                {stage === "warn" && (
                  <div className="flex flex-col gap-3 rounded-lg border border-error/20 bg-error-container/40 p-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 rounded-lg bg-primary px-3 py-2 text-label-md font-semibold text-white transition-colors hover:bg-primary/90"
                      >
                        Keep my plan
                      </button>
                      <button
                        ...
                        className="flex-1 rounded-lg border border-on-surface/15 bg-surface px-3 py-2 text-label-md font-semibold text-on-surface transition-colors hover:bg-on-surface/5"
                      >
                        Continue
                      </button>
                    </div>
                ...
                {stage === "reason" && (
                  <div className="flex flex-col gap-3 rounded-lg border border-on-surface/8 bg-surface-container-lowest p-4">
                    <p className="font-display text-title-sm font-bold text-on-surface">
                      ...
                      {[...CANCEL_REASONS, "Other"].map((reason) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex items-center gap-2 rounded-lg px-3 py-2 text-left text-label-sm transition-colors ${
                              active
                                ? "bg-primary-fixed/70 text-primary"
                                : "border border-on-surface/10 bg-surface text-on-surface/80 hover:bg-on-surface/5"
                            }`}
                          >
                            <span
                              className={`grid h-4 w-4 shrink-0 place-items-center rounded-full border-2 ${
                                active ? "border-primary bg-primary" : "border-on-surface/30"
                              }`}
                            >
                              {active && (
                    ...
                    {selectedReason === "Other" && (
                      <textarea
                        ...
                        rows={3}
                        maxLength={500}
                        className="w-full resize-none rounded-lg border border-on-surface/15 bg-surface px-3 py-2 text-label-sm text-on-surface placeholder:text-on-surface/40 focus:border-primary focus:outline-none"
                      />
                    )}
                    ...
                    <div className="mt-1 flex gap-2">
                      <button
                        ...
                        disabled={loading}
                        onClick={() => setStage("warn")}
                        className="flex-1 rounded-lg border border-on-surface/15 bg-surface px-3 py-2 text-label-md font-semibold text-on-surface/80 transition-colors hover:bg-on-surface/5 disabled:opacity-50"
                      >
                        Back
                      </button>
                      <button
                        ...
                        className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg bg-error px-3 py-2 text-label-md font-semibold text-white transition-colors hover:bg-error/90 disabled:cursor-not-allowed disabled:opacity-50"
                      >
